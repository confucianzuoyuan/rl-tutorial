...
> 2. **for** $k=0,1,2,...$ **do**
> 3.         > 收集一组轨迹 $\mathrm{D}_k=\{\tau_i\}$ ，收集方法：在环境中运行策略 $\pi_l=\pi(\theta_k)$
> 4.         > 计算回报 $\hat{R}_t$
...
graph TD
    ...
    A["$$\max_\theta\mathrm{E}_{q,o_i}\left[\sum_{i=1}^G\min(r_iA_i,\text{clip}(r_i,1-\epsilon,1+\epsilon)A_i)-\beta\mathrm{D}_{KL}(\pi_\theta \vert\vert \pi_\text{ref})\right]$$"]
    end
